...
export default function Volunteer(props) {
  ...
  const handleAddition = (e) => {
    ...
    axios
      .post(
        `http://178.239.151.92:8000/register/register_student/${location.state.username}/${location.state.password}/`,
        sentForm
      )
      .then((res) => {
        if (res.data == "saved") {
          axios
            .get(
              `http://178.239.151.92:8000/register/list/delete/${props.volunteer.id}/${location.state.username}/${location.state.password}/`
            )
            .then((res) => {
              setLoading(false);
  ...
  const handleUpdation = (e) => {
    ...
    axios
      .post(
        `http://178.239.151.92:8000/register/list/update/${props.volunteer.id}/${location.state.username}/${location.state.password}/`,
        sentFormData
      )
      .then((res) => {
        setLoading(false);
